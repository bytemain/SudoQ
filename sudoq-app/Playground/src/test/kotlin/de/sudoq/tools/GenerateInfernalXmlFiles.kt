 ...
 *
 * **MOVED TO PLAYGROUND PROJECT** This tool is separated from the main test suite to avoid running
 * expensive generation operations during normal testing.
 *
 * Run this ONLY when you want to regenerate infernal sudoku templates:
 * ```
...
class GenerateInfernalXmlFiles : GeneratorCallback {
    ...
    private fun saveToXml(sudoku: Sudoku, file: File, sudokuId: Int) {
        ...
        sudoku.cells?.entries?.sortedBy { it.value.id }?.forEach { (position, cell) ->
            if (!cell.isNotSolved) {
                writer.write(
                        "<fieldmap id=\"${cell.id}\" editable=\"true\" solution=\"${cell.solution}\">\n"
                )
